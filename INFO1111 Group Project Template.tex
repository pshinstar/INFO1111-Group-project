...
\textbf{\large
    \begin{tabular}{|p{0.5\textwidth}|p{0.3\textwidth}|p{0.2\textwidth}|}
        ...
        \hline
        Ali Sheng & 520470671 & 1 \\
        Dong Yoon Shin (Patrick) & 520271045 & 1,2 \\
        Lily Weng & 520474864 & 1,2 \\
        Roshni Vadassery & 520462540 & 1,2 \\
        \hline
    \end{tabular}
}\\[0.75cm]
\end{titlepage}
...
The ability to add footnotes provides further context and detail to a block of text, adding more value and dynamics to the document. The \footnote [] command creates an automatically generated footnote \cite{ali8}:

Demonstration of an automatically generated footnote marker\footnote []

whereas \footnote [21] would create:

Demonstration of a footnote with an assigned value\footnote [21]

Similarly, margin notes formats blocks of texts, creating a new environment by adding notes on either the right or the left of text blocks with the \marginpar [left text] {right text} command. Alongside the \raggedright and \raggedleft commands changes the typesetting of margin notes \cite{ali9}.
...
In latex, cross referencing is done by assigning each element, such as a chart or picture, to a number which can later be used when the user wants to reference different parts of the document. The command \ref is used to execute cross referencing. Cross referencing includes two steps. The user must save the elements with the appropriate number for reference, and then it must replace the \ref with the appropriate number. The user must recompile the document twice to see the correct numbering of the elements in order for accurate cross referencing.
Although latex has an abundance of in-built commands which makes coding easier for users, new custom commands are essential as it is useful in minimising repetitive or complex formatting. The \newcommand function is executed to create the new command, and two arguments are required; the name of the new compound and the definition of the command. Additionally, different types of commands can be made. Simple commands which is a basic commands with the name and definition of command. Commands involving parameters can also be created where the name and definition of the command is given, as well as the number of parameters the new command accepts. Other commands include Commands with optional parameters and Overwriting existing commands.
